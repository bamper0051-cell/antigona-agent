"""ClaudeCodeAdapter — delegates tasks to the Claude Code CLI (claude)."""
...
LOGGER = logging.getLogger("antigona.subagents.adapters.claude_code")


class ClaudeCodeAdapter:
    """Subagent adapter that invokes ``claude`` CLI via asyncio subprocess.

    The ``claude`` binary must be on $PATH. The adapter spawns a short-lived
    ...
    async def execute(
        ...
        LOGGER.info(
            "ClaudeCodeAdapter executing task %s (timeout=%ds)",
            execution_id,
        ...
        try:
            ...
            LOGGER.warning("ClaudeCodeAdapter execution %s timed out", execution_id)
    ...
    async def cancel(self, execution_id: str) -> None:
        LOGGER.info("ClaudeCodeAdapter cancel %s (no-op for sync adapter)", execution_id)
