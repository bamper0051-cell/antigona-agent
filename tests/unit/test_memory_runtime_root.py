...
def _tree_signature(path: Path) -> dict[str, tuple[int, int, int] | None]:
    """Signature of direct children of *path* (empty for a missing path).

    Returns a mapping ``name -> (inode, size, mtime_ns)``; a child that cannot
    be ``stat``-ed maps to ``None``.  A directory that does not exist (or
    cannot be listed) yields ``{}``.
    """
    try:
        children = sorted(path.iterdir())
    except OSError:
        return {}
    return {child.name: _path_signature(child) for child in children}
...
def test_no_memory_dir_created_under_source_root(
    ...
    exact set of children, and each one's inode/size/mtime) is snapshotted
    before the call and compared after it.  A *new* creation or write by the
    product still fails the test.
    """
