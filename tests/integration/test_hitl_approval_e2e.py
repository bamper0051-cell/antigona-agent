...
def test_approval_decision_drives_worker_to_done_without_cas_failure(
    ...
    database_url = f"sqlite:///{tmp_path/'db_e2e_approval.sqlite'}"
    ...
    with TestClient(gateway_app) as gateway:
        created = gateway.post(
            "/flows",
            headers={**_auth(), "Idempotency-Key": "e2e-approval-1"},
            json={
                ...
                "goal": "create file after approval",
                "path": "approved_result.env",
                "content": "done via hitl",
            },
        )
        assert created.status_code == 201
        flow_id = created.json()["id"]

        # 1. Worker takes the task, plans, hits HITL -> transitions to WAITING_APPROVAL
        ...
        # 3. Worker re-claims job from queue and finishes execution (must not fail CAS)
        with gateway_app.state.database.session_factory() as session:
            queue = DurableQueue(session)
            job = queue.claim("worker-1", 30)
            assert job is not None
        ...
        # 4. Verify artifact content and DONE state in DB
        with gateway_app.state.database.session_factory() as session:
            task = TaskRepository(session).get(flow_id)
            assert task.status == TaskState.DONE.value

        assert (workspace / "approved_result.env").read_text(encoding="utf-8") == "done via hitl"
