...
# plus every process whose cwd is the checkout, identified by a *redacted*
# fingerprint) so the next pollution can be attributed instead of re-blamed on
# the suite.  It is deliberately read-only: the only snapshot it takes is a
# single ``exists()`` check, so it is cheap and cannot change behaviour for
# other tests.
#
# Secret hygiene (defect B46a): the actor fingerprint published here is
# ``pid`` + ``cwd`` + executable basename + a 12-hex-character SHA-256 prefix
# of the raw command line + its length.  The raw ``/proc/<pid>/cmdline`` must
# never be embedded in the failure text: this report lands in CI job logs and
# in archived evidence, and a command line can carry credentials (``--token=…``,
# ``Authorization: Bearer …``).  The digest keeps cross-run attribution ("same
# actor as last time") without disclosing the line.
#
# Depth (defect B46c): the directory listing walks the tree *recursively* and
# is bounded at ``_MAX_REPORT_ENTRIES``; when the bound is hit an explicit
# ``<walk truncated …>`` line is appended so the evidence never overstates its
# own coverage.

_CHECKOUT_ROOT = Path(__file__).resolve().parents[1]
_CHECKOUT_MEMORY_DIR = _CHECKOUT_ROOT / ".memory"

#: Upper bound on entries materialised by :func:`_iter_tree_entries`.
_MAX_REPORT_ENTRIES = 2000


def _cmdline_fingerprint(cmdline: str) -> str:
    """Non-secret actor fingerprint for one raw process command line.

    Returns ``exec=<basename> cmdline_sha256=<12 hex> cmdline_len=<n>``.  The
    command line itself is never echoed — see the B46a note above.
    """
    text = cmdline.strip()
    digest = hashlib.sha256(text.encode("utf-8", "replace")).hexdigest()[:12]
    if not text:
        return f"exec=<empty> cmdline_sha256={digest} cmdline_len=0"
    argv0 = text.split(" ", 1)[0]
    exec_name = os.path.basename(argv0) or "<unknown>"
    return f"exec={exec_name} cmdline_sha256={digest} cmdline_len={len(text)}"
...
def _checkout_cwd_processes() -> list[str]:
    """``pid cwd`` plus a *redacted* command-line fingerprint per process.

    The command line of a process whose cwd is the checkout is never emitted
    verbatim (B46a): only the executable basename, a 12-character SHA-256
    prefix of the raw line and its length are published, which is enough to
    recognise the actor across runs without leaking a credential that may sit
    on its argv.
    """
    try:
    ...
    for pid in pids:
        ...
        try:
            ...
            found.append(f"pid={pid} cwd={cwd} cmdline=<unavailable>")
            continue
        cmdline = raw.replace(b"\x00", b" ").decode("utf-8", "replace").strip()
        found.append(f"pid={pid} cwd={cwd} {_cmdline_fingerprint(cmdline)}")
    return found


def _iter_tree_entries(
    root: Path, limit: int = _MAX_REPORT_ENTRIES
) -> tuple[list[Path], bool]:
    """Bounded, recursive, cycle-safe listing of *root*.

    Returns ``(entries, truncated)``.  Directories are queued breadth-first and
    symlinks are never followed, so a symlink loop cannot make this quadratic
    or infinite.  ``truncated`` is ``True`` when *limit* entries were already
    collected, in which case the caller must say so in the report.
    """
    entries: list[Path] = []
    truncated = False
...
def _checkout_pollution_report(target: Path) -> str:
    ...
    entries, truncated = _iter_tree_entries(target, _MAX_REPORT_ENTRIES)
    for entry in entries:
        lines.append(f"  entry {_stat_line(entry)}")
    if truncated:
        lines.append(f"  <walk truncated at {_MAX_REPORT_ENTRIES} entries>")
    if not entries and not truncated:
        lines.append("  <no entries listed (empty or unreadable)>")
    processes = _checkout_cwd_processes()
    if processes:
        lines.append("  processes whose cwd is the checkout (redacted fingerprints):")
...
def guard_checkout_memory_dir_pollution() -> Generator[None, None, None]:
    ...
    never fail spuriously.  A pre-existing directory is accepted, but the guard
    is then DISARMED for the session and says so loudly (B46b) instead of
    returning silently: a write into an already-present ``.memory`` can no
    longer be attributed to this run.
    """
    try:
    ...
    if existed_before:
        warnings.warn(
            "checkout memory-dir pollution guard is DISARMED for this session: "
            f"{_CHECKOUT_MEMORY_DIR} already existed at session start, so any "
            "write into it cannot be attributed to this test run (the guard "
            "only fails on a directory it created). Remove the leftover if you "
            "need the guard armed. Directory signature: "
            f"{_stat_line(_CHECKOUT_MEMORY_DIR)}",
            UserWarning,
            stacklevel=1,
        )
        return
