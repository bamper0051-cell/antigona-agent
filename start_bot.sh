...
# Этот скрипт запускает ТОЛЬКО бот с ЧАСТИЧНЫМ env (только .env), что
# является источником расхождений (зависший бот CONFLICT E).
# Отключён по умолчанию. Для осознанного изолированного запуска:
#   ANTIGONA_ALLOW_LEGACY_LAUNCHER=1 ./start_bot.sh
if [ "${ANTIGONA_ALLOW_LEGACY_LAUNCHER:-0}" != "1" ]; then
    echo "⛔ start_bot.sh — LEGACY launcher, отключён (ADR-003). Используйте run.sh." >&2
...
set +a
cd /var/lib/antigona
exec .venv-new/bin/python -m antigona.channels.telegram.bot
